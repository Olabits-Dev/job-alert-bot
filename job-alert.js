...
async function main(){
  ...
  const ranked=jobs
    ...
    .slice(0,12)
    .map(j=>({...j,matchWhy:explainMatch(j),pitch:buildPitch(j)}));
  ...
  await sendDailyReport({
    to:REPORT_TO,
    subject:`Daily Job Bot (${results.applied.length} applied)`,
    html:`Applied: ${results.applied.length}<br>Needs click: ${results.needsClick.length}`
  });
